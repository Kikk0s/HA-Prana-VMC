...
SPEED_DESCRIPTIONS: tuple[PranaSpeedNumberEntityDescription, ...] = (
    PranaSpeedNumberEntityDescription(
        ...
        value_fn=lambda state: state.supply_speed // SPEED_STEP if state.supply_is_on and state.supply_speed else 0,
    ),
    PranaSpeedNumberEntityDescription(
        ...
        value_fn=lambda state: state.extract_speed // SPEED_STEP if state.extract_is_on and state.extract_speed else 0,
    ),
    PranaSpeedNumberEntityDescription(
        ...
        value_fn=lambda state: state.bounded_speed // SPEED_STEP if state.bounded_is_on and state.bounded_speed else 0,
    ),
)
...
class PranaSpeedNumber(PranaEntity, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        # If the device is in bound mode, individual supply/extract controls may be
        # ignored by the unit. Automatically unbind first so the sliders work.
        if (
            self.coordinator.data
            and self.coordinator.data.bound
            and fan_type in (FAN_TYPE_SUPPLY, FAN_TYPE_EXTRACT)
        ):
            _LOGGER.debug(
                "Disabling bound mode before changing %s speed",
                fan_type,
            )
            await self.coordinator.async_set_switch("bound", False)

        # Re-read the latest state after a potential mode change.
        current_state = self.coordinator.data
        current_speed_level = 0
        current_is_on = False
        if current_state:
            current_speed_level = self.entity_description.value_fn(current_state)
            current_is_on = current_state.is_fan_on(fan_type)

        # Skip if the effective level has not changed.
        ...
        # Ensure the fan is on before setting the new speed. Some Prana units ignore
        # the speed change if the specific fan channel is currently off.
